...
import { Loader2, Plus, Pencil, Trash2, Save, PackagePlus } from "lucide-react";
...
const empty = { name: "", unit: "pc", stock_qty: "0", cost_per_unit: "0", low_stock_threshold: "0" };
...
export default function BottlesPage() {
  ...
  const save = async () => {
    ...
    const { error } = editingId
      ? await supabase.from("perfume_bottles").update(payload).eq("id", editingId)
      : await supabase.from("perfume_bottles").insert(payload);
    setSaving(false);
    if (error) {
  ...
  const remove = async (id: number) => {
    ...
    const supabase = createClient();
    await supabase.from("perfume_bottles").delete().eq("id", id);
    load();
  };

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold">{t("perfume.bottlesPage", locale)}</h1>
      <PerfumeNav />

      <div className="rounded-lg border bg-card p-4">
        <h2 className="mb-3 text-sm font-semibold">
          {editingId ? t("perfume.edit", locale) : t("perfume.addBottle", locale)}
        </h2>
